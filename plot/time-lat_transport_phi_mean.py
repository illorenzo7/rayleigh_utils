...
shear_index = np.argmin(np.abs(qvals_ind - 1626))
compr_index = np.argmin(np.abs(qvals_ind - 1627))
adv_index = np.argmin(np.abs(qvals_ind - 1628))
trans_tot_index = np.argmin(np.abs(qvals_ind - 1629))
...
for i in range(len(i_desiredrvals)):
    ...
    savename = dirname_stripped + '_time-lat_transport_p_mean_' +\
