...
for i in range(nargs):
    ...
    if (arg == '-minmax'):
        ...
        my_min_bt = float(args[i+1])
        my_max_bt = float(args[i+2])
...
shear_index_tot = np.argmin(np.abs(qvals_ind - 1606))
compr_index_tot = np.argmin(np.abs(qvals_ind - 1607))
adv_index_tot = np.argmin(np.abs(qvals_ind - 1608))
trans_tot_index_tot = np.argmin(np.abs(qvals_ind - 1609))

shear_index_mean = np.argmin(np.abs(qvals_ind - 1621))
compr_index_mean = np.argmin(np.abs(qvals_ind - 1622))
adv_index_mean = np.argmin(np.abs(qvals_ind - 1623))
trans_tot_index_mean = np.argmin(np.abs(qvals_ind - 1624))
...
bt = vals[:, :, :, bt_index]
shear = vals_ind[:, :, :, shear_index_tot] - vals_ind[:, :, :, shear_index_mean]
compr = vals_ind[:, :, :, compr_index_tot] -  vals_ind[:, :, :, compr_index_mean]
adv = vals_ind[:, :, :, adv_index_tot] - vals_ind[:, :, :, adv_index_mean]
trans_tot = vals_ind[:, :, :, trans_tot_index_tot] -  vals_ind[:, :, :, trans_tot_index_mean]

# Average these traces in time
over2 = navg//2
bt_all = np.zeros((niter - navg + 1, ntheta, nrvals))
...
for i in range(len(i_desiredrvals)):
    ...
    savename = dirname_stripped + '_time-lat_transport_t_fluc_' +\
    ...
    # B_t fluc (prime-prime) production: shear, compression, 
    # advection, total induction, and B_t itself
    ...
    im4 = ax4.pcolormesh(times2, tt_lat2, trans_tot_trace,\
            vmin=my_min_induction, vmax=my_max_induction, cmap='RdYlBu_r')
    im5 = ax5.pcolormesh(times2, tt_lat2, bt_trace,\
    ...
    # Label the plots by induction terms, , , ... B_t
    ax_xmin, ax_xmax = ax1.get_xlim()
    ax_Dx = ax_xmax - ax_xmin
    label1 = r'$[\overline{\mathbf{B}^\prime\cdot\nabla\mathbf{v}^\prime}]_\theta$'
    ax1.text(ax_xmin + 1.01*ax_Dx, 0., label1, va='center',\
            rotation=270)
    label1 = r'$-\overline{B_\theta^\prime(\nabla\cdot\mathbf{v}^\prime)}$'
    ax2.text(ax_xmin + 1.01*ax_Dx, 0.,  label1, va='center',\
            rotation=270)
    label1 = r'$-[\overline{\mathbf{v}^\prime\cdot\nabla\mathbf{B}^\prime}]_\theta$'
    ax3.text(ax_xmin + 1.01*ax_Dx, 0., label1, va='center',\
            rotation=270)
    label1 = r'$[\nabla\times(\overline{\mathbf{v}^\prime\times\mathbf{B}^\prime})]_\theta$'
    ax4.text(ax_xmin + 1.01*ax_Dx, 0.,  label1, va='center',\
            rotation=270)
    ax5.text(ax_xmin + 1.01*ax_Dx, 0.,  r'$\overline{B_\theta}$', va='center',\
            rotation=270)
