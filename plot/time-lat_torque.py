...
if labelbytime:
    plotdir = dirname + '/plots/time-lat' + tag + '_torque_tlabel/'
...
# get the torques
indices = []
indices.append(np.argmin(np.abs(qvals - 1801))) # - Reyn. Stress
indices.append(np.argmin(np.abs(qvals - 1802))) # - advec <v_phi>
indices.append(np.argmin(np.abs(qvals - 1803))) # advec Om_0
indices.append(np.argmin(np.abs(qvals - 1804))) # visc
indices.append(np.argmin(np.abs(qvals - 1819))) # L_z
if magnetism:
    indices.insert(3, np.argmin(np.abs(qvals - 1805))) # Maxwell Mean
    indices.insert(4, np.argmin(np.abs(qvals - 1806))) # Maxwell stress
terms = []
for index in indices:
    terms.append(vals[:, :, :, index])

# remember, Nick's advective forces are negative the real advective forces
# i.e., "v dot grad v" not "advective force prop. - v dot grad v"
terms[0] *= -1
terms[1] *= -1

# calculate the total torque
torque_tot = np.zeros_like(terms[0])
for term in terms[:-1]:
    torque_tot += term

# get the mer circ torque
terms.insert(3, terms[1] + terms[2]) # total mer circ torque

# insert the total torque
terms.insert(-1, torque_tot) 
...
for i in range(len(irvals)):
    ...
    if labelbytime:
        savename = dirname_stripped + '_time-lat_torque_' +\
    ...
    if minmax is None:
        ...
        if len(minmax) == 2:
            ...
            print ("exiting")
            sys.exit()
    mins_and_maxes.append(minmax_Lz)
    ...
    for j in range(nrow):
        ...
        # Plot evolution of each (zonally averaged) term
        if j < nrow - 1:
            units_loc = units
        else:
            units_loc = units_amom
