...
sph_mean_file = get_widest_range_file(datadir, 't_spherical_mean')
az_mean_file = get_widest_range_file(datadir, 't_azimuthal_mean')
t_sph = np.load(datadir + sph_mean_file)
t_az = np.load(datadir + az_mean_file)

t_fluc = t_az - t_sph.reshape((1,nr))
...
std_plus = np.sqrt(np.mean(t_fluc[np.where(t_fluc > 0)]**2))
std_minus = np.sqrt(np.mean(t_fluc[np.where(t_fluc < 0)]**2))
my_min, my_max = -3*std_minus, 3*std_plus
if user_specified_minmax:
    my_min, my_max = user_min, user_max

plot_azav (fig, ax1, t_fluc, rr, cost, sint, plotcontours=False, 
           units = 'K',
           boundstype = 'manual', caller_minmax = (my_min, my_max),\
           norm=MidpointNormalize(0), fsize=16)

savename = plotdir + dirname_stripped + '_t_azimuthal_mean.png'
