...
oldra = False # if True, use the old quantity codes
args = sys.argv[2:]
nargs = len(args)
var_list = ['vr'] # by default, plot just the radial velocity
                  # at each depth
user_specified_depths = False # by default, just plot all depths
                  # user can change this by specifying depth indices
for i in range(nargs):
    ...
    if (arg == '-n'):
        ...
        depth_list = args[i+1].split()
        for j in range(len(depth_list)):
            depth_list[j] = int(depth_list[j])
    elif (arg == '-vars'):
        var_list = args[i+1].split()
...
# Get mean entropy
shellavg_file = get_widest_range_file(datadir, 'Shell_Avgs')
shellavg_vals, shellavg_lut = np.load(datadir + shellavg_file)
s_spherical_mean = shellavg_vals[:, shellavg_lut[501]]

# Get mean velocities
azav_file = get_widest_range_file(datadir, 'AZ_Avgs')
azav_vals, azav_lut = np.load(datadir + azav_file)
vr_av, vt_av, vp_av = azav_vals[:, :, azav_lut[1]], azav_vals[:, :, azav_lut[2]],\
    azav_vals[:, :, azav_lut[3]], 
...
if not user_specified_depths:
    depth_list = list(np.arange(nshells)) # make the default to be plot all depths
...
for varname in var_list:
    ...
    for i_depth in depth_list: # range over each depth
#        for i_depth in range(1): # for debugging purposes
        ...
        vr_slice = a.vals[:, :, i_depth, a.lut[var_indices['vr']], 0]
        vt_slice = a.vals[:, :, i_depth, a.lut[var_indices['vt']], 0]
        vp_slice = a.vals[:, :, i_depth, a.lut[var_indices['vp']], 0]
        vp_slice_prime = vp_slice - (vp_av[:, ir]).reshape((1,nt))
        s_slice = a.vals[:, :, i_depth, a.lut[var_indices['s']], 0]
        p_slice = a.vals[:, :, i_depth, a.lut[var_indices['p']], 0]

        omr_slice = a.vals[:, :, i_depth, a.lut[var_indices['omr']], 0]
        omt_slice = a.vals[:, :, i_depth, a.lut[var_indices['omt']], 0]
        omp_slice = a.vals[:, :, i_depth, a.lut[var_indices['omp']], 0]
