...
# Saves plot in
# [dirname]_induction_r_fluc_[first iter]_[last iter].png
...
# Get induction terms
indices = [1601, 1602, 1603, 1604]
indices_mean = [1616, 1617, 1618, 1619]
# shear, compression, advection, tot. induction
...
for index in indices_mean:
    terms_mean.append(vals[:, :, lut[index]])

# field units and labels
units = r'$\rm{G\ s^{-1}}$'
titles = [r'$[\left\langle\mathbf{B}^\prime\cdot\nabla\mathbf{v}^\prime\right\rangle]_r$',\
...
for iplot in range(nplots):
    ...
    plot_azav (terms[iplot] - terms_mean[iplot], rr, cost, fig=fig, ax=ax,\
        units=units, minmax=minmax, plotcontours=plotcontours, rvals=rvals,\
...
fig.text(margin_x, 1 - 0.3*margin_top, 'r-induction (fluc)',\
         ha='left', va='top', fontsize=fsize, **csfont)
fig.text(margin_x, 1 - 0.5*margin_top, time_string,\
         ha='left', va='top', fontsize=fsize, **csfont)

savefile = plotdir + dirname_stripped + '_induction_r_fluc_' +\
