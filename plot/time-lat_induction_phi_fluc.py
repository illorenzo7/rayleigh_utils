...
plotdir = dirname + '/plots/time-lat' + tag + '_ind_phi_fluc/'
if labelbytime:
    plotdir = dirname + '/plots/time-lat' + tag + '_ind_phi_fluc_tlabel/'
...
# induction terms (phi comp., tot)
indices = []
indices.append(np.argmin(np.abs(qvals - 1611))) # shear 
indices.append(np.argmin(np.abs(qvals - 1612))) # compression
indices.append(np.argmin(np.abs(qvals - 1613))) # advection
indices.append(np.argmin(np.abs(qvals - 1614))) # total induction 
...
# induction terms (phi comp., mean)
indices_mean = []
indices_mean.append(np.argmin(np.abs(qvals - 1626))) # shear 
indices_mean.append(np.argmin(np.abs(qvals - 1627))) # compression
indices_mean.append(np.argmin(np.abs(qvals - 1628))) # advection
indices_mean.append(np.argmin(np.abs(qvals - 1629))) # total induction 
terms_mean = []
...
for i in range(len(irvals)):
    ...
    if labelbytime:
        savename = dirname_stripped + '_time-lat_ind_phi_fluc_' +\
