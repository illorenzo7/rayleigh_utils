...
shear_index_tot = np.argmin(np.abs(qvals_ind - 1611))
compr_index_tot = np.argmin(np.abs(qvals_ind - 1612))
adv_index_tot = np.argmin(np.abs(qvals_ind - 1613))
trans_tot_index_tot = np.argmin(np.abs(qvals_ind - 1614))

shear_index_mean = np.argmin(np.abs(qvals_ind - 1626))
compr_index_mean = np.argmin(np.abs(qvals_ind - 1627))
adv_index_mean = np.argmin(np.abs(qvals_ind - 1628))
trans_tot_index_mean = np.argmin(np.abs(qvals_ind - 1629))
...
# Get induction quantities for B_phi equation
bp = vals[:, :, :, bp_index]
...
for i in range(len(i_desiredrvals)):
    ...
    savename = dirname_stripped + '_time-lat_transport_p_fluc_' +\
