...
shear_index_tot = np.argmin(np.abs(qvals_ind - 1601))
compr_index_tot = np.argmin(np.abs(qvals_ind - 1602))
adv_index_tot = np.argmin(np.abs(qvals_ind - 1603))
trans_tot_index_tot = np.argmin(np.abs(qvals_ind - 1604))

shear_index_mean = np.argmin(np.abs(qvals_ind - 1616))
compr_index_mean = np.argmin(np.abs(qvals_ind - 1617))
adv_index_mean = np.argmin(np.abs(qvals_ind - 1618))
trans_tot_index_mean = np.argmin(np.abs(qvals_ind - 1619))
...
for i in range(len(i_desiredrvals)):
    ...
    savename = dirname_stripped + '_time-lat_transport_r_fluc_' +\
