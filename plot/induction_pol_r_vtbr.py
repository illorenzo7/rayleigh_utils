...
# Saves plot in
# [dirname]_induction_theta_mean_[first iter]_[last iter].png
...
linthreshrz = None
linscalerz = None
the_file = get_widest_range_file(datadir, 'induction_pol_terms')
...
# Get induction terms
ind_off = 0
indices_mm = ind_off + np.arange(0, 16, 4) + 1
indices_pp = ind_off + np.arange(0, 16, 4) + 3
terms = []

term_tot = np.zeros((nt, nr))
count = 0
for index in indices_mm:
    term = vals[:, :, index]
    if count >= 1: # first four terms are the total ones;
               # check how well sum of full breakdown = full induction
        term_tot += term
    terms.append(term)
    count += 1
...
units = r'$\rm{G\ s^{-1}}$'
titles = [r'$-\frac{1}{r\sin\theta}\frac{\partial}{\partial \theta}[\sin\theta\langle v_\theta\rangle\langle B_r\rangle]$',\
r'$-\left\langle\frac{\partial v_\theta}{\partial \theta}\right\rangle\left\langle B_r\right\rangle$',\
r'$-\left\langle v_\theta\right\rangle\left\langle \frac{\partial B_r}{\partial \theta}\right\rangle$',\
r'$-\frac{\cot\theta}{r}\left\langle v_\theta\right\rangle\left\langle B_r\right\rangle$',\
'sum of 3',\

    r'$-\frac{1}{r}\frac{\partial}{\partial\theta}[r\langle v_\theta^\prime B_r^\prime\rangle]$',\
    r'$-\left\langle \frac{\partial v_\theta^\prime}{\partial\theta} B_r^\prime\right\rangle$',\
    r'$-\left\langle v_\theta^\prime \frac{\partial B_r^\prime}{\partial\theta}\right\rangle$',\
r'$-\frac{\cot\theta}{r}\left\langle v_\theta^\prime B_r^\prime\right\rangle$',\
'sum of 3']
...
nplots = len(terms)
ncol = 5 # put three plots per row
...
fig.text(margin_x, 1 - 0.3*margin_top, 'r-induction (vt-br cor)',\
         ha='left', va='top', fontsize=fsize, **csfont)
fig.text(margin_x, 1 - 0.5*margin_top, time_string,\
         ha='left', va='top', fontsize=fsize, **csfont)

savefile = plotdir + dirname_stripped + '_induction_pol_r_vtbr_' +\
