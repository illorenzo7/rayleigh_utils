...
if the_file is None:
    the_file = get_widest_range_file(datadir, 'mag_energy_mm_exact')
...
# sum the (tot) radial + latitudinal generation terms
terms = []
for i in range(5):
    terms.append(vals[:, :, i+15] + vals[:, :, i+20])
terms.append(terms[-2] + terms[-1])
# field units and labels
units = r'$\rm{erg\ s^{-1}\ cm^{-3}}$'
titles = ['shear (pol.)', 'comp (pol.)', 'advec (pol.)', 'tot. ind. (pol.)', 'diff (pol.)', 'sum']

# Set up the actual figure from scratch
fig_width_inches = 12. # TOTAL figure width, in inches
...
nplots = len(terms)
ncol = 6 # put three plots per row
...
fig.text(margin_x, 1 - 0.3*margin_top, '<B_pol> production terms (mmm)',
         ha='left', va='top', fontsize=fsize, **csfont)
fig.text(margin_x, 1 - 0.5*margin_top, time_string,\
         ha='left', va='top', fontsize=fsize, **csfont)

savefile = plotdir + dirname_stripped + '_mag_energy_mean_pol_mmm_' +\
        str(iter1).zfill(8) + '_' + str(iter2).zfill(8) + tag + '.png'

if saveplot:
    print ('Saving work terms at ' + savefile)
