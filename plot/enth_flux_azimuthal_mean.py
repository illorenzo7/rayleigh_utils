...
std_plus = np.sqrt(np.mean(eflux[np.where(eflux > 0)]**2))
std_minus = np.sqrt(np.mean(eflux[np.where(eflux < 0)]**2))
my_min, my_max = -3*std_minus, 3*std_plus
if (user_specified_minmax):
    my_min, my_max = user_min, user_max

plot_azav (fig, ax1, eflux, rr, cost, sint, plotcontours=False, 
           units = r'$\frac{\rm{erg}}{\rm{cm}^2\ \rm{s}}$',
           boundstype = 'manual', caller_minmax = (my_min, my_max),\
           norm=MidpointNormalize(0), fsize=16)

savename = plotdir + dirname_stripped + '_enth_flux_azimuthal_mean.png'
