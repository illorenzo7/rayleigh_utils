...
terms = []
for i in range(5):
    terms.append(vals[:, :, i+30] + vals[:, :, i+35])
...
fig.text(margin_x, 1 - 0.3*margin_top, '<B_pol> production terms (ppp)',
         ha='left', va='top', fontsize=fsize, **csfont)
fig.text(margin_x, 1 - 0.5*margin_top, time_string,\
         ha='left', va='top', fontsize=fsize, **csfont)

savefile = plotdir + dirname_stripped + '_mag_energy_mean_pol_ppp_' +\
