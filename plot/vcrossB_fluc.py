...
linthreshrz = None
linscalerz = None
the_file = get_widest_range_file(datadir, 'vcrossB')
...
# Get the terms:
print ('Getting vcrossB terms from ' + datadir + the_file)
...
units = r'$\rm{G\ cm\ s^{-1}}$'
titles = [r'$\left\langle\mathbf{v}^\prime\times\mathbf{B}^\prime\right\rangle_r$', r'$\left\langle\mathbf{v}^\prime\times\mathbf{B}^\prime\right\rangle_\theta$', r'$\left\langle\mathbf{v}^\prime\times\mathbf{B}^\prime\right\rangle_\phi$', 'vt bp', '-vp bt', 'vp br', '- vr bp', 'vr bt', '- vt br']
...
fig.text(margin_x, 1 - 0.3*margin_top, 'v x B (fluc)',\
         ha='left', va='top', fontsize=fsize, **csfont)
fig.text(margin_x, 1 - 0.5*margin_top, time_string,\
         ha='left', va='top', fontsize=fsize, **csfont)

savefile = plotdir + dirname_stripped + '_vcrossB_fluc_' +\
