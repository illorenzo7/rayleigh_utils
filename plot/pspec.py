...
number_from_end = 1 # By default, average the pspec to end on the first-from-last
                        # (second-to-last)
args = sys.argv[2:]
nargs = len(args)
...
# Get mean entropy
s_spherical_mean = np.load(datadir + 's_spherical_mean.npy')
...
a = ShellSlice(radatadir + files[-number_from_end], '')
...
#        for i_depth in range(1): # for debugging purposes
            ...
            vr_slice = a.vals[:, :, i_depth, a.lut[1], i_time]
            vt_slice = a.vals[:, :, i_depth, a.lut[2], i_time]
            vp_slice = a.vals[:, :, i_depth, a.lut[3], i_time]
            vp_slice_prime = vp_slice - (vp_av[:, ir]).reshape((1,nt))
            s_slice = a.vals[:, :, i_depth, a.lut[64], i_time]

            try:
                omr_slice = a.vals[:, :, i_depth, a.lut[103], i_time]
                omt_slice = a.vals[:, :, i_depth, a.lut[104], i_time]
                omp_slice = a.vals[:, :, i_depth, a.lut[105], i_time]
            except:
                pass
