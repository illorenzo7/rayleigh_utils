...
shear_index = np.argmin(np.abs(qvals_ind - 1621))
compr_index = np.argmin(np.abs(qvals_ind - 1622))
adv_index = np.argmin(np.abs(qvals_ind - 1623))
trans_tot_index = np.argmin(np.abs(qvals_ind - 1624))
...
for i in range(len(i_desiredrvals)):
    ...
    savename = dirname_stripped + '_time-lat_transport_t_mean_' +\
