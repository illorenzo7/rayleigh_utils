...
ind_tot_num = fact*np.sum(vals_me[:, :, 0:3], axis=2)
ind_mmm_num = fact*np.sum(vals_me[:, :, 3:6], axis=2)
ind_ppp_num = fact*np.sum(vals_me[:, :, 6:9], axis=2)
ind_rest_num = ind_tot_num - ind_mmm_num - ind_ppp_num

terms.append(ind_tot_num)
terms.append(ind_mmm_num)
terms.append(ind_ppp_num)
terms.append(ind_rest_num)

# field units and labels
units = r'$\rm{erg\ s^{-1}\ cm^{-3}}$'
titles = ['tot (exact)', 'mmm (exact)', 'fff (exact)', 'the rest (exact)',\
    'tot (numderv)', 'mmm (numderiv)', 'fff (numderiv)',\
    'the rest (numderiv)']
