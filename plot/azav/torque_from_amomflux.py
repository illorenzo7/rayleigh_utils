...
torque_rs_fromf = -(drad(f_rs_r, rr) + 2.*f_rs_r/rr + (1./rr)*(dth(f_rs_t, tt) + cott*f_rs_t) )

torque_mc_fromf = -(drad(f_mc_r, rr) + 2.*f_mc_r/rr + (1./rr)*(dth(f_mc_t, tt) + cott*f_mc_t) )

torque_visc_fromf = -(drad(f_v_r, rr) + 2.*f_v_r/rr + (1./rr)*(dth(f_v_t, tt) + cott*f_v_t) )
...
if magnetism:
    ...
    torque_ms_fromf = -(drad(f_ms_r, rr) + 2.*f_ms_r/rr + (1./rr)*(dth(f_ms_t, tt) + cott*f_ms_t) )
    torque_mm_fromf = -(drad(f_mm_r, rr) + 2.*f_mm_r/rr + (1./rr)*(dth(f_mm_t, tt) + cott*f_mm_t) )
...
torques = [torque_rs, torque_rs_fromf, torque_mc, torque_mc_fromf,\
        torque_visc, torque_visc_fromf]
titles = [r'$\tau_{\rm{rs}}$', r'$\tau_{\rm{rs,\ f}}$', r'$\tau_{\rm{mc}}$', r'$\tau_{\rm{mc,\ f}}$', r'$\tau_{\rm{v}}$', r'$\tau_{\rm{v,\ f}}$']

units = r'$\rm{g}\ \rm{cm}^{-1}\ \rm{s}^{-2}$'

if magnetism:
    torques.append(torque_ms)
    torques.append(torque_ms_fromf)
    torques.append(torque_mm)
    torques.append(torque_mm_fromf)

    titles.append(r'$\tau_{\rm{ms}}$') 
    titles.append(r'$\tau_{\rm{ms,\ f}}$') 
    titles.append(r'$\tau_{\rm{mm}}$') 
    titles.append(r'$\tau_{\rm{mm,\ f}}$') 
...
savefile = plotdir + dirname_stripped + '_torque_fromf_' + str(iter1).zfill(8) +\
