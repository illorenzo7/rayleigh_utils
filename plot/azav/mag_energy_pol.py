...
ind_tot = fact*np.sum(vals[:, :, 0:2], axis=2)
ind_mmm = fact*np.sum(vals[:, :, 3:5], axis=2)
ind_ppp = fact*np.sum(vals[:, :, 6:8], axis=2)
...
fig.text(margin_x, 1 - 0.3*margin_top, 'inductive work pol. (' + which + ')',\
         ha='left', va='top', fontsize=fsize, **csfont)
fig.text(margin_x, 1 - 0.5*margin_top, time_string,\
         ha='left', va='top', fontsize=fsize, **csfont)

savefile = plotdir + dirname_stripped + '_mag_energy_pol_' + which + '_' +\
