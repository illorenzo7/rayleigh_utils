...
from common import get_widest_range_file, strip_dirname, get_file_lists, get_desired_range
...
user_specified_n = False # by default, plot all available slices (set later)
user_specified_start_iter = False
args = sys.argv[2:]
nargs = len(args)
varname = 'vr'
depth = 0 # by default, grab the slice closest to outer boundary
use_old_lut = False
...
if '-range' in args or '-centerrange' in args:
    index_first, index_last = get_desired_range(int_file_list, args)
else:
    index_first = 0
    index_last = nfiles - 1 # by default, plot all files
...
a0 = reading_function(radatadir + file_list[0], '')
t0 = a0.time[0]

# Make the directory to save the plots if it doesn't exist
savedir = plotdir + 'sslice/timeseries/' + varname + '/'
if (not os.path.isdir(savedir)):
    os.makedirs(savedir)
    
print ('about to plot iterations %s through %s ...' %(file_list[index_first],\
    file_list[index_last]))
print ('saving plots to ' + savedir + ' ...')

made_first_plot = False
for ifile in range(index_first, index_last + 1):
    print ('Reading ' + radatadir + file_list[ifile] + ' ...')
    a = reading_function(radatadir + file_list[ifile], '')
    i_depth = np.argmin(np.abs(a.radius - r_depth))
    for iiter in range(a.niter):
        iter_loc = a.iters[iiter]
        t_loc = a.time[iiter]
        t_days = (t_loc - t0)/86400
        
        vr_slice = a.vals[:, :, i_depth, a.lut[qindex_vr], iiter]
        vt_slice = a.vals[:, :, i_depth, a.lut[qindex_vt], iiter]
        vp_slice = a.vals[:, :, i_depth, a.lut[qindex_vp], iiter]
        vp_slice_prime = vp_slice - (vp_av[:, i_depth]).reshape((1, nt))
        s_slice = a.vals[:, :, i_depth, a.lut[qindex_s], iiter]
        
        try: # I didn't always output the vorticities
            omr_slice = a.vals[:, :, i_depth, a.lut[qindex_omr], iiter]
            omt_slice = a.vals[:, :, i_depth, a.lut[qindex_omt], iiter]
            omp_slice = a.vals[:, :, i_depth, a.lut[qindex_omp], iiter]
        except:
            pass
        ...
        if (varname == 'vr'):
         ...
         # cylindrical vorticities
            ...
            sslice = omp_slice - vt_slice + vt_slice/a.radius[i_depth]
         ...
         # velocities/vorticities-squared (vsq --> (m/s)^2)
            ...
            omp = omp_slice - vt_slice + vt_slice/a.radius[i_depth]
            sslice = omp**2 + omr_slice**2 + omt_slice**2
        
         # the entropy fluctuation
        elif (varname == 's'):
            sslice = s_slice - s_spherical_mean[a.inds[i_depth]]
        
        # velocity and entropy (self-) correlations
        elif (varname == 'vrs'):
            sslice = (s_slice - s_spherical_mean[a.inds[i_depth]])*vr_slice/100.
        elif (varname == 'vrvp'):
        ...
        if not made_first_plot:
            glob_min, glob_max = plot_ortho(fig, ax, sslice, lats, lons, a.radius[i_depth], ri, ro,\
                   cax=cax, cbar_units=units[varname], posdef=posdef, return_minmax=True)
            made_first_plot = True
        else:
            plot_ortho(fig, ax, sslice, lats, lons, a.radius[i_depth], ri, ro,\
                   cax=cax, cbar_units=units[varname], posdef=posdef, minmax=(glob_min, glob_max))
        plt.title(labels[varname] + ', ' +\
                  (r'$\ r/r_o=%.3f,\ t=%03.0f$' %((r_loc/ro), t_days)) + ' days')
