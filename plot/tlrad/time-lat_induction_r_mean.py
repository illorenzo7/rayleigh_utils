...
plotdir = dirname + '/plots/time-lat' + tag + '_ind_r_mean/'
if labelbytime:
    plotdir = dirname + '/plots/time-lat' + tag + '_ind_r_mean_tlabel/'
...
indices = []
indices.append(np.argmin(np.abs(qvals_ind - 1616))) # shear 
indices.append(np.argmin(np.abs(qvals_ind - 1617))) # compression
indices.append(np.argmin(np.abs(qvals_ind - 1618))) # advection
indices.append(np.argmin(np.abs(qvals_ind - 1619))) # total induction 
indices.append(np.argmin(np.abs(qvals_ind - 1620))) # diffusion
...
for i in range(len(irvals)):
    ...
    if labelbytime:
        savename = dirname_stripped + '_time-lat_ind_r_mean_' +\
