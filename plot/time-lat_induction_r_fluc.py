...
plotdir = dirname + '/plots/time-lat' + tag + '_ind_r_fluc/'
if labelbytime:
    plotdir = dirname + '/plots/time-lat' + tag + '_ind_r_fluc_tlabel/'
...
# induction terms (r comp., tot)
indices = []
indices.append(np.argmin(np.abs(qvals - 1601))) # shear 
indices.append(np.argmin(np.abs(qvals - 1602))) # compression
indices.append(np.argmin(np.abs(qvals - 1603))) # advection
indices.append(np.argmin(np.abs(qvals - 1604))) # total induction 
...
# induction terms (r comp., mean)
indices_mean = []
indices_mean.append(np.argmin(np.abs(qvals - 1616))) # shear 
indices_mean.append(np.argmin(np.abs(qvals - 1617))) # compression
indices_mean.append(np.argmin(np.abs(qvals - 1618))) # advection
indices_mean.append(np.argmin(np.abs(qvals - 1619))) # total induction 
terms_mean = []
...
labels = [r'$[\left\langle\mathbf{B}^\prime\cdot\nabla\mathbf{v}^\prime\right\rangle]_r$',\
    r'$-\left\langle B_r^\prime(\nabla\cdot\mathbf{v}^\prime)\right\rangle$',\
    r'$-[\left\langle\mathbf{v}^\prime\cdot\nabla\mathbf{B}^\prime\right\rangle]_r$',\
    r'$[\nabla\times(\left\langle\mathbf{v}^\prime\times\mathbf{B}^\prime\right\rangle)]_r$']
...
for i in range(len(irvals)):
    ...
    if labelbytime:
        savename = dirname_stripped + '_time-lat_ind_r_fluc_' +\
