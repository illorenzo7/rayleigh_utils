...
savename = 'lmvals_om_trace-' + file_list[index_first] + '_' +\
...
for i in range(index_first, index_last + 1):
    ...
    for j in range(local_ntimes):
        vals_omr_loc = spec.vals[:, :, :, lut[301], j]
        vals_omt_loc = spec.vals[:, :, :, lut[302], j]
        vals_omp_loc = spec.vals[:, :, :, lut[303], j]
        vals_omr.append(vals_omr_loc.tolist())
        vals_omt.append(vals_omt_loc.tolist())
        vals_omp.append(vals_omp_loc.tolist())
        times.append(spec.time[j])
        iters.append(spec.iters[j])

# Convert lists into arrays
vals_omr = np.array(vals_omr)
vals_omt = np.array(vals_omt)
vals_omp = np.array(vals_omp)
...
f = open(savefile, 'wb')
pickle.dump({'vals_omr': vals_omr, 'vals_omt': vals_omt, \
        'vals_omp': vals_omp, 'times': times, 'iters': iters, \
