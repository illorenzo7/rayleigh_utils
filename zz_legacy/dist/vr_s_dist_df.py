...
# Created: 02/25/2018
# This script computes the 2-D distribution for v_r and S' (for the downflows
# only) at each point in the nt x nr meridional plane. 
# Requires the data produced by dist/bininfo/vr_s_df.py stored in 
# vrs_df_binedges.npy, vrs_df_binspacing.npy
# Outputs the distribution as (nt, nr, nbins_vr, nbins_s) (big file!) at 
# [dirname]_vr_s_dist_df_[first iter]_[last iter].npy
...
savename = dirname_stripped + '_vr_s_dist_df_' + file_list[index_first] + '_' +\
    file_list[index_last - 1] + '.npy'
savefile = datadir + savename  

# Get the bin information from the output of dist/bininfo/vr_s.py
vr_binedges, s_binedges = np.load(datadir + 'vrs_df_binedges.npy')
vr_space, s_space = np.load(datadir + 'vrs_df_binspacing.npy')
...
vr_s_dist_df = np.zeros((nt, nr, nbins_vr, nbins_s))
        ...
        for tindex in range(mer.niter):
            ...
            for ir in range(nr):
                ...
                for it in range(nt):
                    ...
                    vr_s_dist_df[it,ir,bin_index_vr,bin_index_s] += 1.

print ('Saving the vr_vp distribution at ' + savefile + ' ...')
np.save(savefile,vr_s_dist_df)