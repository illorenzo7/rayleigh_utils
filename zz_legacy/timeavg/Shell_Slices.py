...
if rank == 0:
    ...
    f = open(savefile, 'wb')
    pickle.dump({'vals': vals, 'lut': a0.lut, 'count': ntimes, 'iter1': iter1, 'iter2': iter2, 'qv': a0.qv, 'nq': a0.nq,  'rvalscm': rvalscm, 'nr': nr,  'tt': tt, 'tt_lat': tt_lat, 'sint': sint, 'cost': cost,'nt': nt, 'nphi': nphi}, f, protocol=4)
    f.close()
    t2 = time.time()
    print (format_time(t2 - t1))
