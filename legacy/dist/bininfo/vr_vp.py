...
# in computing the joint v_r and v_phi distribution. The bins are equally spaced,
# and determined from the datafile 
# [dirname]_vr_vp_dist_limits_full_[first iter]_[last iter].npy, produced by
# the script dist/limits/vr_vp.py. nbins_vr/nbins_vp (set to 100 each today)
# set the number of bins to be used in each direction.
# The script outputs 
# (vr_binedges, vp_binedges) in vrvp_binedges.npy (shapes (nr, nbins_var + 1]))
# (vr_bincenters, vp_bincenters) in vrvp_bincenters.npy (shapes (nr, nbins_var))
# and (vr_space, vp_space) in vrvp_binspacing.npy (shapes (nr,))
...
limits_file = get_widest_range_file(datadir, 'vr_vp_dist_limits_full')
print ('Getting dist limits from ' + datadir + limits_file)
vr_maxes, vr_mins, vp_maxes, vp_mins = np.load(datadir + limits_file)
vr_maxes *= 1.05
vr_mins *= 1.05
vp_maxes *= 1.05
vp_mins *= 1.05
