...
# saved in the file
# [dirname]_vr_s_dist_limits_full_[first iter]_[last iter].npy
...
savename = dirname_stripped + '_vr_s_dist_limits_full_' + \
...
for i in range(index_first, index_last):
    ...
    for pindex in range(nphi):
        for tindex in range(mer.niter):
            ...
            vr_loc = mer.vals[pindex,:,:,mer.lut[1],tindex]/100. 
            ...
            s_maxes_loc = np.max(s_fluc, axis=0)
            s_mins_loc = np.min(s_fluc, axis=0)
...
np.save(savefile,(vr_maxes, vr_mins, s_maxes, s_mins))
