...
if rank == 0:
    ...
    kw_default['rmin'] = None # can adjust location of tangent cylinder
    kw_default['rmax'] = None 
...
if rank == 0:
    ...
    rmax_tc = kw.rmax
    meta = [\
dirname, dataname, radatadir, latvals, rvals, omega0, rho, rmin_tc, rmax_tc]
else:
    meta = None
dirname, dataname, radatadir, latvals, rvals, omega0, rho, rmin_tc, rmax_tc = comm.bcast(meta, root=0)
...
for i in range(my_nfiles):
    ...
    sint_2d = np.sin(tt_2d) # needed for beta_topo
    ...
    for j in range(a.niter):
        ...
        # get specific profiles we need

        # <uphi> and its derivatives
        uphi = vals_loc[:, :, a.lut[3]]
        uphi_dl = dlambda(uphi, rr, tt)
        uphi_dl2 = dlambda(uphi_dl, rr, tt)
        uphi_dt = 1./rr_2d*dth(uphi, tt)
        uphi_dt2 = 1./rr_2d*dth(uphi_dt, tt)

        # square velocity amplitude
        rke = vals_loc[:, :, a.lut[402]]
        tke = vals_loc[:, :, a.lut[403]]
        pke = vals_loc[:, :, a.lut[404]]
        usq = 2*(rke + tke + pke)/rho.reshape((1, nr)) # total

        # beta and beta_topo
        # 2D version
        beta_topo = compute_beta_topo(rr_2d, sint_2d, rmin=rmin_tc, rmax=rmax_tc)
        beta = sint_2d/rr_2d

        # Rhines scale profiles
        L_R_lambda = np.sqrt(np.sqrt(usq)/np.abs(beta_topo))
        L_R_theta = np.sqrt(np.sqrt(usq)/np.abs(beta))

        # jet scale profiles
        L_jet_lambda = np.sqrt(np.abs(uphi/uphi_dl2))
        L_jet_theta = np.sqrt(np.abs(uphi/uphi_dt2))

        # Get the values in the separate quadrants
        vals_gav = np.zeros((4, nquadlat, nquadr))
        ...
        for ilat in range(nquadlat): # remember: tt_lat is INCREASING
            ...
            for ir in range(nquadr): # remember: rvals increase but r-inds decrease
                ...
                L_R_lambda_gav = np.sum(L_R_lambda[it1:it2+1,ir1:ir2+1]*weights_quad)
                L_R_theta_gav = np.sum(L_R_theta[it1:it2+1,ir1:ir2+1]*weights_quad)
                L_jet_lambda_gav = np.sum(L_jet_lambda[it1:it2+1,ir1:ir2+1]*weights_quad)
                L_jet_theta_gav = np.sum(L_jet_theta[it1:it2+1,ir1:ir2+1]*weights_quad)

                vals_gav[:, ilat, ir] = L_R_lambda_gav, L_R_theta_gav, L_jet_lambda_gav, L_jet_theta_gav
...
if rank == 0:
    ...
    basename = 'jetrhines_trace_nquadlat%i_nquadr%i' %(nquadlat, nquadr)
    if not rmin_tc is None:
        basename += '_rmin%.3f' %rmin_tc
    if not rmax_tc is None:
        basename += '_rmax%.3f' %rmax_tc
