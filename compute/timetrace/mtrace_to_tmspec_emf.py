...
kwargs_default = dict({'irvals': np.array([0]), 'rvals': None, 'mmax': None, 'nonlin': None})
...
print (buff_line)
print ("converting %i data file(s)" %(len(irvals)*5))
print ("irvals = ", irvals)

# loop over rvals and emf types and convert data
for irval in irvals:
    # convert each of the emf files separately
    for emftag in ['emfr', 'emfr_nodr', 'emft', 'emft_nodr', 'emfp']:
        dataname_mtrace = 'mtrace_' + emftag + '_irval%02i' %irval +\
                clas0['tag']
