...
dummy, nphi, nrvals, nq = np.shape(di_list[0]['vals'])
vals = np.zeros((niter, nphi, nrvals, nq))
...
savename = dirname_stripped + '_time-longitude_clat%s%02.0f_dlat%03.0f_'\
        %(hemisphere, np.abs(clat), dlat) + tag + str(iter1).zfill(8) +\
