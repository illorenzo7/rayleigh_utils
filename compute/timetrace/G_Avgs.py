...
if rank == 0:
    ...
    if nproc > 1: # can't receive from myself!
        ...
        gf = G_Avgs(radatadir + file_list[0], '')
        nrec_last = gf.niter
    nrec_full = g0.niter
    ...
    for i in range(my_nfiles):
        print ("rank %i analyzing G_Avgs/%s"\
                %(rank, str(my_files[i]).zfill(8)))
        if i == 0:
            g = g0
        else:   
    ...
    print ("rank ", rank, "received nrec_full")
...
if rank == 0:
    ...
    f = open(savefile, 'wb')
    pickle.dump({'vals': vals, 'times': times, 'iters': iters, 'lut': g0.lut, 'count': ntimes, 'iter1': iter1, 'iter2': iter2, 'qv': g0.qv, 'nq': g0.nq},\
            f, protocol=4)
    f.close()
    t2 = time.time()
