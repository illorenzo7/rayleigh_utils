...
savename = dirname_stripped + '_s_spherical_mean_' + file_list[index_first] + '_' +\
...
entropy = np.zeros(nr)
print ('Considering G_Avg files %08i through %08i for the average ...'\
       %(int_file_list[index_first], int_file_list[index_last - 1]))
count = 0
for ii in range (index_first, index_last):
    print ('Adding G_Avgs/%s to the average...' %file_list[ii])
    shellavg = Shell_Avgs(radatadir + file_list[ii], '')
...
print ('Saving spherical entropy mean at ' + savefile + ' ...')
np.save(savefile, entropy)
