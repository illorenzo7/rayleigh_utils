...
# in computing the joint v_r and S' distribution for the downflows only.
# The bins are equally spaced, and determined from the datafile 
# [dirname]_vr_s_dist_limits_df_[first iter]_[last iter].npy, produced by
# the script dist/limits/vr_s_df.py. nbins_vr/nbins_s (set to 100 each today)
# set the number of bins to be used in each direction.
# The script outputs 
# (vr_binedges, s_binedges) in vrs_df_binedges.npy (shapes (nr, nbins_var + 1]))
# (vr_bincenters, s_bincenters) in vrs_df_bincenters.npy (shapes (nr, nbins_var))
# and (vr_space, s_space) in vrs_df_binspacing.npy (shapes (nr,))
...
limits_file = get_widest_range_file(datadir, 'vr_s_dist_limits_df')
print ('Getting dist limits from ' + datadir + limits_file)
vr_mins, s_maxes, s_mins = np.load(datadir + limits_file)
s_maxes *= 1.05
vr_mins *= 1.05
s_mins *= 1.05
...
np.save(datadir + 'vrs_df_binedges.npy', (vr_binedges, s_binedges))
np.save(datadir + 'vrs_df_bincenters.npy', (vr_bincenters, s_bincenters))
np.save(datadir + 'vrs_df_binspacing.npy', (vr_space, s_space))