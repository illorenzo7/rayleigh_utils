...
# Created: 05/28/2021
##################################################################
# This routine computes the time average of B-B correlations from
...
for i in range(my_nfiles):
    ...
    for j in range(niter -1, -1, -1): # go last to first in case "niters" 
        ...
        my_vals[:, :, ind_off_full + 4] += np.mean(br*bp, axis=0)*my_weight 
        my_vals[:, :, ind_off_full + 5] += np.mean(bt*bp, axis=0)*my_weight
        ...
        my_vals[:, :, ind_off_mean + 4] += np.mean(br_m*bp_m, axis=0)*my_weight 
        my_vals[:, :, ind_off_mean + 5] += np.mean(bt_m*bp_m, axis=0)*my_weight
        # fluc terms
        ind_off_fluc = 12
        for k in range(6):
