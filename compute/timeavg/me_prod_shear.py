...
# SHEAR components: (B_r, B_theta, B_phi) * (derivs v), 2 curvature terms
# 6 x 3 x 5 = 90 terms
##################################################################
...
for i in range(my_nfiles):
    ...
    for j in range(a.niter):
        ...
        # SHEAR break up: 3 b dot v terms, + 2 curvature terms
        count = 0 
        for k in range(6): 
            ...
            for l in range(3):
                shear1 = bb_loc[0]*dvdx_loc[l][0]
                shear2 = bb_loc[1]*dvdx_loc[l][1]
                shear3 = bb_loc[2]*dvdx_loc[l][2]
                ...
                my_vals[:, :, count] = np.mean(bb1_loc[l]*shear1, axis=0)*my_weight
                count += 1
                my_vals[:, :, count] = np.mean(bb1_loc[l]*shear2, axis=0)*my_weight
                count += 1
                my_vals[:, :, count] = np.mean(bb1_loc[l]*shear3, axis=0)*my_weight
                count += 1
