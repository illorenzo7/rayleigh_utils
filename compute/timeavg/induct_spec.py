...
for i in range(my_nfiles):
    ...
    for direc in range(3):
        bfield = np.conjugate(a.vals[:,:,:,a.lut[801 + direc],:])
        # "old" induction terms (should be unchanged)
        induct_set = [801 + direc, 1604 + 5*direc, 1605 + 5*direc]
        # "alternate" terms
        alt_terms = 3001 + np.arange(0, 12, 3) + direc
        induct_set += alt_terms.tolist()
