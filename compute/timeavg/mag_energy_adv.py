...
for i in range(my_nfiles):
    ...
    for j in range(a.niter):
        ...
        # compute induction advective terms
        ...
        ind_t += -vt*dbtdt
        ind_t += -vp*dbtdp
        ind_t += -(1./rr_3d)*(vt*br - cott_3d*vp*bp)

        # full phi
        ind_p = -vr*dbpdr
        ind_p += -vt*dbpdt
        ind_p += -vp*dbpdp
        ind_p += -(1./rr_3d)*(vp*br + cott_3d*vp*bt)

        # mean radial
        ind_r_m = -vr_m*dbrdr_m
        ind_r_m += -vt_m*dbrdt_m
        ind_r_m += -vp_m*dbrdp_m
        ind_r_m += 1./rr_3d*(vt_m*bt_m + vp_m*bp_m)

        # mean theta
        ind_t_m = -vr_m*dbtdr_m
        ind_t_m += -vt_m*dbtdt_m
        ind_t_m += -vp_m*dbtdp_m
        ind_t_m += -(1./rr_3d)*(vt_m*br_m - cott_3d*vp_m*bp_m)

        # mean phi
        ind_p_m = -vr_m*dbpdr_m
        ind_p_m += -vt_m*dbpdt_m
        ind_p_m += -vp_m*dbpdp_m
        ind_p_m += -(1./rr_3d)*(vp_m*br_m + cott_3d*vp_m*bt_m)
...
if rank == 0:
    ...
    savename = dirname_stripped + '_mag_energy_adv_' +\
