...
savename = dirname_stripped + '_s_azimuthal_mean_' + file_list[index_first] + '_' +\
...
for ii in range(index_first, index_last):
    ...
    for jj in range(local_ntimes):
        entropy_loc = az.vals[:, :, az.lut[64],jj]
        entropy += entropy_loc
        count += 1

entropy /= count

print ('Saving azimuthal entropy mean at ' + savefile + ' ...')
