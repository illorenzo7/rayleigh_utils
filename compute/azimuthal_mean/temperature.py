...
pressure_file = get_widest_range_file(datadir, 'p_azimuthal_mean')
pressure = np.load(datadir + pressure_file)

entropy_file = get_widest_range_file(datadir, 's_azimuthal_mean')
...
p_ref = (ref.pressure).reshape((1, nr))
t_ref = (ref.temperature).reshape((1, nr))
...
savename = dirname_stripped + '_t_azimuthal_mean_' + str(iter1).zfill(8) + '_' + str(iter2).zfill(8) + '.npy'
savefile = datadir + savename    

print ('Saving azimuthal temperature mean at ' + savefile + ' ...')
