...
print("(make sure that 'sigma' = sqrt(Pr*buoy) is set to %1.3f" %np.sqrt(kw.buoy*di_par.pr) )
print("in main_parameters, for consistency with the nondimensionalization.)")
...
f.write("sigma : %1.3f\n" %kw.sigma)
f.write("buoy : %1.3f\n" %kw.buoy)
f.write("(make sure that 'sigma' = sqrt(Pr*buoy) is set to %1.3f\n" %np.sqrt(kw.buoy*di_par.pr) )
f.write("in main_parameters, for consistency with the nondimensionalization.)\n")
f.write("delta_kappa : %1.5f\n" %kw.delta)
f.write(buff_line)
f.write(buff_line + '\n')
f.close()
