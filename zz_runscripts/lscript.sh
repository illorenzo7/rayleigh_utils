...
echo "select=$(($nprocs/$ncpus + 1*($nprocs%$ncpus)))"
select=$(($nprocs/$ncpus + 1*($nprocs%$ncpus)))

nprow=`python $rau/zz_runscripts/nprow.py $ncpus`
npcol=`python $rau/zz_runscripts/npcol.py $ncpus`
...
echo "#PBS -l site=needed=/home1+/nobackupp11" >> lscript
